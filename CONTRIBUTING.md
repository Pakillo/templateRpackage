...
First off, thanks for taking the time to contribute to `templateRpackage`!
...
We use the [Tidyverse style guide](https://style.tidyverse.org/) for writing R code. We recommend that you install [air](https://github.com/posit-dev/air) to automatically format your R code.

The project `templateRpackage` uses the `lower_snake_case` for naming objects (variables and functions).
...
For instance, for a contribution on the README: `doc: add reference in README`
...
2. [Clone](https://docs.github.com/en/repositories/creating-and-managing-repositories/cloning-a-repository) your fork using `git clone fork-url` (replace `fork-url` by the URL of your fork). Alternatively, open RStudio/Positron IDE and create a New Project from Version Control.
...
Do not forget to replace `Vignette Title` (twice) by the title of your new vignette.
